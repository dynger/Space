class_name CameraMovement extends Camera
...
func _update_mouselook(delta):
	if Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		# prevents looking up/down beyond comprehension
...
func _on_Button_toggled(button_pressed):
	...
	if !telescope_pressed:
		fov = 4
		telescope_pressed = true
	else:
		fov = 60
		telescope_pressed = false
